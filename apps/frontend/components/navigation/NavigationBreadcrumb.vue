    ...
    <ol class="flex items-center gap-2 overflow-hidden">
      ...
      >
        ...
        <component
          ...
          class="focus-visible:ring-primary-500 inline-flex max-w-[7.5rem] items-center truncate rounded-lg px-2 py-1 text-sm font-medium transition hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-white sm:max-w-none dark:hover:bg-gray-800 dark:hover:text-white dark:focus-visible:ring-offset-gray-900"
  ...
  rawBreadcrumb.value.forEach((item, index, source) => {
    if (item.type === 'part') {
      ...
      items.push({
        ...item,
        to: fallbackLesson
          ? { name: 'lessons-code', params: { code: fallbackLesson.code } }
          : undefined,
      })
      return
    }
      ...
      items.push({
        ...
        to:
          index === source.length - 1
            ? undefined
            : { name: 'lessons-code', params: { code: item.id } },
