    ...
    <Transition name="modal">
      ...
      >
        <div
          ...
          role="dialog"
          aria-modal="true"
          :aria-labelledby="`qr-modal-title-${work.id}`"
        >
          <div class="mb-4 flex items-start justify-between">
            <h3
              :id="`qr-modal-title-${work.id}`"
              class="text-lg font-semibold text-gray-900 dark:text-gray-100"
            >
              Share Student Work
            </h3>
            <button
              class="focus:ring-primary-500 rounded text-gray-400 transition-colors hover:text-gray-600 focus:outline-none focus:ring-2 dark:hover:text-gray-300"
              aria-label="Close"
              @click="close"
            >
...
function generateQrCode(work: StudentWork) {
...
function getWorkUrl(work: StudentWork): string {
  const baseUrl = window.location.origin
  return `${baseUrl}/students?work=${work.slug || work.id}`
}
...
function downloadQr() {
  ...
  canvas.toBlob((blob) => {
    ...
    link.href = url
    link.download = `qr-code-${props.work.slug || props.work.id}.png`
    link.click()
