  ...
  <div class="masonry-grid">
    ...
    >
      <div class="relative overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-800">
        ...
        <div
          v-else
          class="flex h-64 w-full items-center justify-center bg-gray-200 dark:bg-gray-700"
        >
        ...
        >
          <div class="absolute bottom-0 left-0 right-0 p-4 text-white">
            <h3 class="mb-1 text-lg font-semibold">{{ work.studentName }}</h3>
            <div class="flex items-center gap-2 text-sm">
              <span v-if="work.discipline" class="rounded bg-white/20 px-2 py-0.5">
                {{ work.discipline }}
              </span>
              <span v-if="work.loop" class="rounded bg-white/20 px-2 py-0.5">
                {{ work.loop }}
              </span>
              <span v-if="work.grade" class="rounded bg-white/20 px-2 py-0.5">
                {{ work.grade }}
              </span>
            </div>
            <p v-if="work.description" class="mt-2 line-clamp-2 text-sm">
              {{ work.description }}
            </p>
          </div>
        </div>
        ...
        <div
          ...
          class="bg-primary-500 absolute right-2 top-2 rounded px-2 py-1 text-xs font-semibold text-white"
        >
          对比
        </div>
      </div>
