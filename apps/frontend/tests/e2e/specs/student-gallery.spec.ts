...
test.describe('Student Gallery', () => {
  ...
  test('should display student works grid', async ({ page }) => {
    ...
    const grid = page.locator(
      '[data-test="gallery-grid"], [data-test="works-grid"], .grid, .gallery'
    )
    ...
    const workItems = page.locator(
      '[data-test="work-item"], .work-card, [data-test="student-work"]'
    )
  ...
  test('should handle empty gallery state', async ({ page }) => {
    ...
    if ((await mediumFilter.count()) > 0) {
      await mediumFilter
        .first()
        .selectOption({ label: /nonexistent/i })
        .catch(() => {})
      await page.waitForTimeout(500)
    }
  })
