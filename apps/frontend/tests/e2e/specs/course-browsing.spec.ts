...
test.describe('Course Browsing', () => {
  ...
  test('should display lesson navigation sidebar', async ({ page }) => {
    ...
    const lessonLinks = page
      .locator('a[href*="lesson"], a[href*="/"]')
      .filter({ hasText: /Foundation|Core|Extended/i })
  ...
  test('should display breadcrumb navigation', async ({ page }) => {
    ...
    const lessonLink = page
      .locator('a')
      .filter({ hasText: /Welcome|Foundation/i })
      .first()
    if (await lessonLink.isVisible()) {
      ...
      const breadcrumb = page.locator(
        '[data-test="breadcrumb"], nav[aria-label*="breadcrumb" i], .breadcrumb'
      )
  ...
  test('should switch difficulty levels', async ({ page }) => {
    ...
    const lessonLink = page
      .locator('a')
      .filter({ hasText: /Color|Composition/i })
      .first()
    if (await lessonLink.isVisible()) {
      await lessonLink.click()
      await page.waitForLoadState('networkidle')
      ...
      const difficultySelector = page
        .locator('[data-test="difficulty-selector"], select, button')
  ...
  test('should render markdown content properly', async ({ page }) => {
    ...
    const lessonLink = page
      .locator('a')
      .filter({ hasText: /Welcome|Line|Color/i })
      .first()
    await lessonLink.click()
    await page.waitForLoadState('networkidle')
